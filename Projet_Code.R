...
# LISA
locm <-localmoran(gfrance85$Suicides, cont.lw)
locm

# Diagramme de Moran (sans utiliser la fonction)
# standardisation de la variable
x <- gfrance85$Suicides
zx <- (x - mean(x))/sd(x)
mean(zx)
var(zx)
# creation du decalage spatial
wzx <- lag.listw(cont.lw,zx)
# calcul de la constante et de la pente (I de Moran) en utilisant lm
morlm <- lm(wzx ~ zx)
aa <- morlm$coefficients[1]
mori <- morlm$coefficients[2]
aa
mori
# representation graphique en utilisant plot:
par(pty="s") # make sure it’s square
plot(zx,wzx,xlab="Suicides",ylab="Spatial Lag of Sucides")
abline(aa,mori)
abline(h=0,lty=2)
abline(v=0,lty=2)
...
# _________________________ AIC des modeles ____________________________
